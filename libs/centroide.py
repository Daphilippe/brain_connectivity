...
def compute(source='./data/R/',directory='variables/'):
    ...
    for i in numpy_vars:
        ...
        tools.save_value(L2_val,'L2_val_'+str(i[9:17]),directory+str(i[9:17]))
        tools.save_value(L2_name,'L2_name_'+str(i[9:17]),directory+str(i[9:17]))
...
def display(data_source='./data/R/',centroide_source='./variables/R/',directory='variables/'):
    ...
    L_trie=[L_name[i][9:] for i in np.argsort(L_val)]
