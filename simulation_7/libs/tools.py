...
@list of functions 
- data_generator_simulation1()
- data_generator_simulation2(rand_seed=0,noise=0.1)

- continue2discret(Img_xs,Img_xt,seuil=10e-4)
- extract_point(S)
- degree(xs,xt,G,degree=1)

- label_amplitude(a,b)
- label_position(xs,xt)

- save_value(value,title,directory='')
- save_fig(title,directory='')

- unnormalized_kernel(points, factor=10)
- estimate_pseudo_density(points, grid_size=101, factor=10)
...
def extract_point(Img_xs):
  ...
  for i in range(h):
    for j in range(w):
      if Img_xs[i,j]>0:
        xs.append(np.array([j,i]))
        a.append(Img_xs[i,j])
  a=a/sum(a)#normalisation
  
  return np.array(xs),np.array(a)
